...
def render_app():
    ...
    render_sidebar()
    if st.session_state[get_session_key("mode")] == "child":
...
def render_sidebar():
    with st.sidebar:
        ...
        st.markdown("---")
        mode = st.radio(t("sidebar.mode"), ["👶 " + t("sidebar.child_mode"), "👨‍👩‍👧 " + t("sidebar.parent_mode")], 
...
def render_child_mode(tokenizer, model):
    ...
    with col1:
        ...
        if get_session_key("chat_history") not in st.session_state:
            st.session_state[get_session_key("chat_history")] = [{"role": "assistant", "content": f"{t('child.greeting')} {question}"}]
...
def render_parent_mode(tokenizer, model, rag_docs, font_path):
    ...
    render_charts(today_data, pos_ratio, positive, negative)  # Added positive and negative
...
def render_metrics(total, positive, negative, pos_ratio, avg_confidence):
    ...
    with col1: st.markdown(f'<div class="metric-card"><div class="metric-label">{t("parent.metric.total")}</div><div class="metric-value" style="color: #6366f1;">{total}</div><div class="metric-label">{t("parent.metric.count")}</div></div>', unsafe_allow_html=True)
    with col2: st.markdown(f'<div class="metric-card"><div class="metric-label">{t("parent.metric.positive")}</div><div class="metric-value" style="color: #10b981;">{positive}</div><div class="metric-label">{t("parent.metric.percent", value=pos_ratio)}%</div></div>', unsafe_allow_html=True)
    with col3: st.markdown(f'<div class="metric-card"><div class="metric-label">{t("parent.metric.negative")}</div><div class="metric-value" style="color: #ef4444;">{negative}</div><div class="metric-label">{t("parent.metric.percent", value=100-pos_ratio)}%</div></div>', unsafe_allow_html=True)
    with col4:
        ...
        st.markdown(f'<div class="metric-card"><div class="metric-label">{t("parent.metric.mood")}</div><div class="metric-value" style="color: {color}; font-size: 1.5rem;">{mood}</div><div class="metric-label">{t("parent.metric.confidence", value=avg_confidence)}%</div></div>', unsafe_allow_html=True)

def render_charts(today_data, pos_ratio, positive, negative):  # Added positive and negative parameters
    ...
    with col1:
        ...
        fig.update_layout(xaxis_title=t("parent.chart.time"), yaxis=dict(tickvals=[-1, 0, 1], ticktext=[t("parent.chart.negative"), t("parent.chart.neutral"), t("parent.chart.positive")]), height=300, showlegend=False, hovermode='x', plot_bgcolor='white', margin=dict(l=0, r=0, t=0, b=0))
        st.plotly_chart(fig, use_container_width=True)
        st.markdown('</div>', unsafe_allow_html=True)
    with col2:
        st.markdown('<div class="content-card">', unsafe_allow_html=True)
        st.markdown("### 🎨 " + t("parent.chart.mood"))
        fig = go.Figure(go.Indicator(mode="gauge+number", value=pos_ratio, domain={'x': [0, 1], 'y': [0, 1]}, title={'text': t("parent.chart.positive_index"), 'font': {'size': 16}}, gauge={'axis': {'range': [0, 100]}, 'bar': {'color': "#6366f1"}, 'bgcolor': "white", 'borderwidth': 2, 'bordercolor': "gray", 'steps': [{'range': [0, 50], 'color': '#fee2e2'}, {'range': [50, 100], 'color': '#d1fae5'}], 'threshold': {'line': {'color': "red", 'width': 4}, 'thickness': 0.75, 'value': 90}}))
        fig.update_layout(height=200, margin=dict(l=0, r=0, t=30, b=0), paper_bgcolor='rgba(0,0,0,0)', font={'color': "#4a5568"})
        st.plotly_chart(fig, use_container_width=True)
        st.metric(t("parent.metric.positive_short"), positive)  # Now uses the passed parameter
        st.metric(t("parent.metric.negative_short"), negative)  # Now uses the passed parameter
        st.markdown('</div>', unsafe_allow_html=True)
...
def render_conversations(today_data):
    ...
    for i, item in enumerate(today_data[-3:]):
        ...
        st.markdown(f'<div style="margin-bottom: 1rem;"><div style="display: flex; align-items: center; gap: 0.5rem; margin-bottom: 0.5rem;"><span style="color: #64748b; font-size: 0.9rem;">{time}</span><span class="emotion-tag {tag_class}" aria-label="{emotion_text}">{emotion_text}</span></div><div style="background: #f8fafc; padding: 0.75rem; border-radius: 8px; font-size: 0.9rem;">{item["answer"][:50]}{"..." if len(item["answer"]) > 50 else ""}</div></div>', unsafe_allow_html=True)
    st.markdown('</div>', unsafe_allow_html=True)
